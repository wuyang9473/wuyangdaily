...
if [ -f /etc/docker/daemon.json ]; then
    if command -v jq &>/dev/null; then
        current_mirror=$(jq -r '.["registry-mirrors"]? // [] | .[0] // "无"' /etc/docker/daemon.json 2>/dev/null)
        echo "$current_mirror"
    else
        current_mirror=$(grep -oP '"registry-mirrors":\s*\[\s*"\K[^"]+' /etc/docker/daemon.json 2>/dev/null || echo "无")
        echo "$current_mirror"
    fi
else
echo "未配置"
...
echo "---------------------------------------------------"
    ...
    echo "当前可选的加速器："
    echo "1) 阿里云（需手动输入你的专属加速器地址）"
    echo "2) 轩辕镜像 (https://docker.xuanyuan.me)  ⭐推荐"
    echo "3) DaoCloud 镜像 (https://docker.m.daocloud.io)  ⭐推荐"
    echo "4) 毫秒镜像 (https://docker.1ms.run)"
    echo "5) 1Panel 镜像加速 (https://docker.1panel.live)"
    echo "6) CNIX 镜像 (https://docker.m.ixdev.cn)"
    echo "7) 耗子面板 (https://hub.rat.dev)"
    echo "8) DockerProxy (https://dockerproxy.net)"
    echo "9) 自定义地址"
    echo "0) 取消"
    read -r -p "请选择: " mirror_opt

    local mirror_url=""
    case $mirror_opt in
        1)
            echo "🔑 获取阿里云加速器地址：登录 https://cr.console.aliyun.com -> 镜像工具 -> 镜像加速器"
            read -r -p "请输入加速器地址（如 https://xxxx.mirror.aliyuncs.com）: " mirror_url
            ;;
        2) mirror_url="https://docker.xuanyuan.me" ;;
        3) mirror_url="https://docker.m.daocloud.io" ;;
        4) mirror_url="https://docker.1ms.run" ;;
        5) mirror_url="https://docker.1panel.live" ;;
        6) mirror_url="https://docker.m.ixdev.cn" ;;
        7) mirror_url="https://hub.rat.dev" ;;
        8) mirror_url="https://dockerproxy.net" ;;
        9)
            read -r -p "请输入自定义加速器地址（含 http:// 或 https://）: " mirror_url
            ;;
        0) echo "已取消"; pause_return ;;
        *) echo "⚠️ 无效选择"; pause_return ;;
    esac
    ...
    if command -v jq &>/dev/null; then
        tmp_file=$(mktemp)
        if [ -f /etc/docker/daemon.json ]; then
            jq --arg url "$mirror_url" '.["registry-mirrors"] = [$url]' /etc/docker/daemon.json > "$tmp_file"
        else
            jq -n --arg url "$mirror_url" '{ "registry-mirrors": [$url] }' > "$tmp_file"
        fi
...
EOF
    ...
    echo "📝 写入配置: $mirror_url"
    echo "🔄 重启 Docker 服务..."
    systemctl restart docker || echo "⚠️ Docker 重启失败，请手动执行 systemctl restart docker"

    echo "💡 当前镜像加速器配置"
    docker info 2>/dev/null | grep -A1 'Registry Mirrors' || echo "⚠️ 未检测到加速器配置，请检查"
    pause_return
}
...
if command -v last &> /dev/null; then
    ...
    last -i -n 3 | grep -vE "reboot|wtmp" | awk '{printf "%s   %s   %s   %s %s %s %s\n", $1, $2, $3, $4, $5, $6, $7}' | sed \
        -e 's/Mon/星期一/g' -e 's/Tue/星期二/g' -e 's/Wed/星期三/g' -e 's/Thu/星期四/g' -e 's/Fri/星期五/g' -e 's/Sat/星期六/g' -e 's/Sun/星期日/g' \
        -e 's/Jan/一月/g' -e 's/Feb/二月/g' -e 's/Mar/三月/g' -e 's/Apr/四月/g' -e 's/May/五月/g' -e 's/Jun/六月/g' -e 's/Jul/七月/g' -e 's/Aug/八月/g' -e 's/Sep/九月/g' -e 's/Oct/十月/g' -e 's/Nov/十一月/g' -e 's/Dec/十二月/g'
fi
